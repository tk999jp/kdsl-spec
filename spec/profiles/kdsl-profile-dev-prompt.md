# KDSL Profile: dev-prompt v3.5-kanji-agent
...
同一契約意味→正本1回定義→意味名参照
同一対象の固定要素群→意味保持可能なら意味束化
意味束定義後→成功／作業／検証／停止／報告で構成要素再列挙×
意味束で新条件生成×／条件差ある要素の無理な束化×
成功条件:=最終成立状態
検証:=成立確認方法
成功否定→停止へ全複製×
section独立可読性目的の再掲禁止
...
dense:=min成立保持＋章最小＋正本参照・意味束化積極化＋作業文語幹化＋自然文残留最小
```
