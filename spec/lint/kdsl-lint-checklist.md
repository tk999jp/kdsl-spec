...
同一の公開不変／候補一致／対象外不変／承認済等を複数sectionへ再展開
成功条件／検証／保持／停止条件／報告で同一成立状態を全文再説明
成功条件の否定形を停止条件へ全複製
section単独可読性目的で正本値／意味を再掲
意味明確な短名で参照可能なのに長値／意味反復
```

意味束不足:

```text
同一対象の固定要素群を各sectionで個別再列挙
意味名で参照可能なのにmain／tag、Package／size／hash等を反復展開
...
```text
U明示／canonicalにない新条件を束化で生成
条件差のある要素を一束へ統合して意味弱化
曖昧一字aliasで複数意味を混同
```
...
```text
U明示／canonical条件から導いた観測を独立成功条件へ昇格
派生観測を独立停止条件／保持条件へ昇格
既存禁止の証明詳細を新規契約として増殖
```
...
```text
正本短名をinline code内で未展開literalの実行commandとして使用
shell変数未定義なのに短名をcommand引数へ直接挿入
```
...
派生観測がU明示／canonical根拠なしに新規契約へ昇格していない
下位情報が上位契約を上書きしていない
