# KDSL Standalone Converter v1.5
...
意味保持済み不要section→省略
section数維持を目的化×
権限／承認境界／非対象／禁止→状態差保持可能な範囲で統合可
保持→成功条件／正本で完全保持済みなら省略可
報告→標準R1で足りるなら`報告:R1`; Task固有追加だけ追記
```

成功条件／検証／停止条件の役割差は保持するが、同じ値／意味を再展開しない。
...
dense:=min成立保持＋章・箇条書き最小＋正本参照・意味束化積極化＋section閉包＋作業文語幹化＋自然文残留最小
```
