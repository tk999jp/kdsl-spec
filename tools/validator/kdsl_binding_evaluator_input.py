...
REQUIRED_FACTS = (
    ...
    'contract_source_ref', 'evaluated_at', 'evaluator_ref', 'repository_state_ref',
    'stop', 'preconditions',
)
...
def evaluate_inputs(contract_text: str, k1_text: str, pf1_text: str, facts: dict[str, Any]) -> tuple[dict[str, Any] | None, str, list[str]]:
    ...
    errors.extend('contract: ' + item for item in contract.errors)
    errors.extend('K1: ' + item for item in k1.errors)
    errors.extend('PF1: ' + item for item in pf1.errors)
    if errors or contract.model is None or k1.model is None or pf1.model is None:
        return None, '', errors

    compatibility = resolve_runtime_control(k1, pf1)
    errors.extend('runtime-control: ' + item for item in compatibility.errors)
